...
std::string Sha256(const std::filesystem::path& path)
{
    ...
    { BCryptCloseAlgorithmProvider(algorithm, 0); return "unavailable"; }
    unsigned char buffer[65536], digest[32];
    bool ok = true;
    while (file.read((char*)buffer, sizeof(buffer)) || file.gcount())
        if (BCryptHashData(hash, buffer, (ULONG)file.gcount(), 0) < 0) { ok = false; break; }
    ok = ok && !file.bad() && BCryptFinishHash(hash, digest, sizeof(digest), 0) >= 0;
    BCryptDestroyHash(hash); BCryptCloseAlgorithmProvider(algorithm, 0);
    if (!ok) return "unavailable";
    std::string result;
    for (auto byte : digest) result += std::format("{:02X}", byte);
    return result;
}
void FileReport(const std::filesystem::path& path, const char* kind, bool hashFile)
{
    ...
    struct CachedHash { uintmax_t size; std::filesystem::file_time_type modified; std::string hash; };
    ...
    LOG_INFO("NR diagnostic {}: {} size={} version={} SHA256={}", kind, path.string(),
             size,
             hasVersion ? std::format("{}.{}.{}", version.major, version.minor, version.patch) : "unknown",
             digest);
}
}
...
void RuntimeReport(ID3D12GraphicsCommandList* commands, ID3D12Device* device, const char* phase)
{
    ...
    try
    {
        LOG_INFO("NR diagnostic {}: device={} commands={} thread={}", phase, (void*)device,
                 (void*)commands, GetCurrentThreadId());
        if (device)
        {
            ...
            LOG_INFO("NR diagnostic: adapter LUID={:08X}:{:08X}, device status=0x{:08X}, command device={} "
                     "query=0x{:08X}, command adapter={:08X}:{:08X}, nodes={}", (unsigned)luid.HighPart,
                     luid.LowPart, (unsigned)device->GetDeviceRemovedReason(), (void*)commandDevice.Get(),
                     (unsigned)hr, (unsigned)commandLuid.HighPart, commandLuid.LowPart, device->GetNodeCount());
        }
        ...
        for (const auto& path : paths) FileReport(path, "runtime candidate", true);
        if (const auto module = GetModuleHandleW(L"nvngx_dlssnr.dll"))
        {
            ...
            if (length && length < std::size(path)) FileReport(path, "loaded runtime", true);
            else LOG_INFO("NR diagnostic: runtime module found but its path is unavailable");
        }
        else LOG_INFO("NR diagnostic: nvngx_dlssnr.dll is not present in the process module list at {}", phase);
    }
    catch (const std::exception& e) { LOG_INFO("NR diagnostic: runtime inspection unavailable: {}", e.what()); }
}
}
