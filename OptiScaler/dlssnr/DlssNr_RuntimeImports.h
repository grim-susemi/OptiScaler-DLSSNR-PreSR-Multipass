...
{
    slots.clear();
    auto range = [&](size_t offset, size_t length) { return offset <= image.size() && length <= image.size() - offset; };
    if (!range(0, sizeof(IMAGE_DOS_HEADER))) return false;
    const auto* dos = reinterpret_cast<const IMAGE_DOS_HEADER*>(image.data());
    if (dos->e_magic != IMAGE_DOS_SIGNATURE || dos->e_lfanew < 0 ||
        !range(dos->e_lfanew, sizeof(IMAGE_NT_HEADERS64))) return false;
    ...
    if (!directory.VirtualAddress || !range(directory.VirtualAddress, directory.Size)) return false;
    ...
    {
        const auto* descriptor = reinterpret_cast<const IMAGE_IMPORT_DESCRIPTOR*>(image.data() + directory.VirtualAddress + offset);
        if (!descriptor->Name && !descriptor->FirstThunk && !descriptor->OriginalFirstThunk)
        { slots = std::move(found); return true; }
        if (!descriptor->FirstThunk) return false;
        ...
        for (size_t index = 0;; ++index)
        {
            ...
            if (!range(nameOffset, sizeof(IMAGE_THUNK_DATA64)) || !range(slotOffset, sizeof(IMAGE_THUNK_DATA64))) return false;
            const auto entry = reinterpret_cast<const IMAGE_THUNK_DATA64*>(image.data() + nameOffset)->u1.AddressOfData;
            if (!entry) break;
            if (IMAGE_SNAP_BY_ORDINAL64(entry)) continue;
            if (!range(entry, sizeof(WORD) + 1)) return false;
            ...
            {
                if (slotOffset % alignof(void*)) return false;
