...
try
{
    ...
    data.pSysMem = model.data();
    check(device->CreateTexture2D(&desc, &data, &modelTex));
    ...
    check(device->CreateTexture2D(&desc, nullptr, &outputTex));
    check(device->CreateTexture2D(&desc, nullptr, &keepTex));
    ...
    {
        DlssNrConstants s {};
        s.Mode = DlssNrMode_Resolve;
        s.Width = kSize;
        s.Height = kSize;
        s.WhitePoint = 1;
        s.Passthrough = 1;
        s.ApplyModel = 1;
        s.ReversibleMode = reversible;
        s.TransferStrength = s.ColourStrength = 1;
        s.MaxRatio = 2;
        s.SkinDetail = s.SkinColour = s.EnvironmentDetail = s.EnvironmentColour = 1;
        s.ReplaceDetailStrength = strength;
        s.ModelWorkScale = workScale;
        return s;
    };
    ...
    const auto identicalToModel = [&](const std::vector<Pixel>& image)
    {
        for (const Pixel& p : image)
            if (!closeTo(p.r, 0.6f, 1e-4f) || !closeTo(p.g, 0.4f, 1e-4f) || !closeTo(p.b, 0.2f, 1e-4f))
                return false;
        return true;
    };
    const auto identical = [&](const std::vector<Pixel>& a, const std::vector<Pixel>& b)
    {
        for (size_t i = 0; i < a.size(); ++i)
            if (!closeTo(a[i].r, b[i].r, 1e-5f) || !closeTo(a[i].g, b[i].g, 1e-5f) || !closeTo(a[i].b, b[i].b, 1e-5f))
                return false;
        return true;
    };
    ...
    for (const Target& target :
         { Target { "HLSL source", fromSource.Get() }, Target { "production blob", production.Get() } })
    {
        ...
        expect(identicalToModel(run(target.shader, makeSettings(2, 1.0f, 1.0f))),
               "full-size model changed Replace output");
        expect(identicalToModel(run(target.shader, makeSettings(2, 1.0f, 2.0f))),
               "supersampled model changed Replace output");
        expect(identicalToModel(run(target.shader, makeSettings(2, 1.0f, 0.0f))),
               "unset ModelWorkScale changed Replace output");
        ...
        // Bounded contrast in stops preserves a positive model response at dark edges.
        const Pixel dark = at(on, 4, 4);
        const float darkRatio = std::exp2(-0.12f / (0.32f + 1.0f / 512));
        ...
        const float brightRatio = std::exp2(0.12f / (0.8f + 1.0f / 512));
        ...
        // The PR's ratio clipped this near-black edge to zero at the default strength.
        auto contrast = original;
        for (int y = 0; y < kSize; ++y)
            for (int x = 0; x < kSize; ++x)
            {
                const float value = x <= 4 ? 0.01f : 1.0f;
                contrast[y * kSize + x] = { value, value, value, 1 };
            }
        ctx->UpdateSubresource(originalTex.Get(), 0, nullptr, contrast.data(), sizeof(Pixel) * kSize, 0);
        const auto nearBlack = run(target.shader, makeSettings(2, 0.5f, 0.5f));
        expect(at(nearBlack, 4, 4).r > 0.3f, "near-black edge crushed to black");
        ctx->UpdateSubresource(originalTex.Get(), 0, nullptr, original.data(), sizeof(Pixel) * kSize, 0);
